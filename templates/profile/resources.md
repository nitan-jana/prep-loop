...
Sources considered and rejected, with the reason. Written down so the same
decision is not remade every few weeks, and so a change of target can revisit
it deliberately.
...
Nothing paywalled is copied into this repo. An inventory of a paid source is
built by the user against their own account and holds identifiers and titles
so a question can be named — never the contents.
