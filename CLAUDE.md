...
project — but only while the left column stays clean.
[`tools/leak-check.ts`](tools/leak-check.ts) is what keeps it true. Run it
before committing anything on the left.
