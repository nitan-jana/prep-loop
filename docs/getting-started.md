...
Then open an agent CLI in the directory — Claude Code, Codex, Cursor and Gemini
CLI each have a wrapper in the repo, and any other agent reads
[`AGENTS.md`](../AGENTS.md) — and run `/preploop`, which reports where the
install stands and names the one thing to run next. On a fresh clone that is
`/preploop onboard`.

`/preploop` is safe at any moment. It reads state and prints it, and it is the only
session here that writes nothing and starts nothing.

**A session that has modes prints them when run bare, and waits.** `/preploop onboard`
and `/preploop loop` both do this rather than guessing which mode was meant. So
...
`/preploop onboard` creates `instance/`, the folder holding everything about the person
using it, and interviews them into a profile. Four stages, each a sitting of its
own, resumable across sessions.

Run it bare to see the four with their status and which is next. `/preploop onboard
...
`/preploop onboard verify` re-runs verification later, when a source that was missing
...
`/preploop plan` writes the week and mirrors every block to the calendar in the same
...
`/preploop checkin` closes the day. It gathers evidence first, then quizzes what the
...
Coming back after time away, open with `/preploop`. It says what is missing and
names one thing, rather than leaving you to work out where you stopped.

## A review

`/preploop loop` runs several rounds back to back from a brief prepared ahead, then
...
Two sittings, and it will not merge them. `/preploop loop prepare` writes the brief
ahead of the day; `/preploop loop run` conducts the rounds and scores them. A brief
...
`/preploop mock` runs a single deep-dive round on its own, opening on whichever claim has
the least evidence behind it.

`/preploop story` turns one experience into a usable story with the follow-ups an
...
`/preploop recall` asks many short questions instead of a few deep ones, drawn oldest
...
**No session commits.** Commands write files and stop, so the working tree is
...
Edit `policy/`. The commands are thin and point at it; the rules about grading,
